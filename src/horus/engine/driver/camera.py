...
class Camera(object):
    ...
    def _check_driver(self):
        ...
        if system == 'Windows':
            i = 9
            mean = 255
            while mean > 200 and i < 100:
                i += 1
                self.set_exposure(i)
                frame = self.capture_image(flush=1)
                mean = int(sum(cv2.mean(frame)) / 3.0)
            if mean > 200:
                raise WrongDriver()
        ...
        if self._is_connected:
            if self._contrast != value:
                ...
                if system == 'Darwin':
                    ...
                    value = int(int(value) / self._max_contrast)
        ...
        if self._is_connected:
            if self._saturation != value:
                ...
                if system == 'Darwin':
                    ...
                    value = int(int(value) / self._max_saturation)
        ...
        if self._is_connected:
            if self._exposure != value or force:
                ...
                if system == 'Darwin':
                    ...
                    value = int(int(value) / self._max_exposure)

...
class Camera(object):
    ...
    def _set_width(self, value):
        self._expected_width = value
        self._capture.set(cv2.CAP_PROP_FRAME_WIDTH, value)

    def _set_height(self, value):
        self._expected_height = value
    ...
    def _update_resolution(self):
        ...
        if -1 == self._width or -1 == self._height:
            frame = self.capture_image(5, False)
            if frame is not None:
                self._width = int(frame.shape[0])
                self._height = int(frame.shape[1])

...
def save_point_cloud_stream(stream, point_cloud):
    frame = b"ply\n"
    frame += b"format binary_little_endian 1.0\n"
    frame += b"comment Generated by Horus software\n"
    frame += "element vertex {0}\n".format(len(point_cloud)).encode('utf-8')
    frame += b"property float x\n"
    frame += b"property float y\n"
    frame += b"property float z\n"
    frame += b"property uchar red\n"
    frame += b"property uchar green\n"
    frame += b"property uchar blue\n"
    frame += b"element face 0\n"
    frame += b"property list uchar int vertex_indices\n"
    frame += b"end_header\n"
